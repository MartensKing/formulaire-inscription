...
function validerPassword(valeur) {
    ...
    if (valeur.length <= 7) {
        return { valide: false, message: "le mot de passe ne contient pas assez de caractere" }
    }
    ...
    if (!/[A-Z]/.test(valeur)) {
        return { valide: false, message: "le mot de passe doit contenir au moins une lettre majiscule" }
    }
    //si toutes les conditions sont remplis le mot de passe est valider
